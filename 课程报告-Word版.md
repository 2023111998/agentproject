...
[TABLE]
维度	指标	采集来源	评分公式
效率	P95 响应延迟	实测采样（100 次 HTTP 请求 x 5 端点）	<100ms -> A+ / <300ms -> A / <1000ms -> B / <3000ms -> C / >=3000ms -> D
可用性	请求成功率	Docker healthcheck + 实测采样	成功率 = 200响应数 / 总请求数 x 100%，>=99.99% -> A+ / >=99.9% -> A / >=99.0% -> B / >=95.0% -> C / <95.0% -> D
健壮性	容错能力	Resilience4j 断路器状态 + 护栏拦截率	断路器保持 close + 护栏拦截生效 -> A+
吞吐率	资源占用效率	docker stats 实时采集	低内存占用（<400MB）且 CPU <1% 空闲时 -> A+
[/TABLE]

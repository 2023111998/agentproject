...
```
+-- 前端层 (Vue 3 CDN + Leaflet) --+
|  customer.html | merchant.html | rider.html | index.html |
+-----------------+---------------+------------+-----------+
                              | HTTP REST (Nginx :80)
+-- 应用层 --+     +-- 应用层 --+
| campus-server-1 | | campus-server-2 |  Spring Boot :8000 x2
+--------+--------+ +--------+--------+
         +--------+----------+
                  |
+-- 领域层 (核心) -------------------------------------------+
|  AgentOrchestrator -> router() -> 物流/售后/导购            |
|  BpmnEngine (StAX) + BpmnHandlers (8 handlers)             |
|  RagService (n-gram TF) | Guardrails (注入/越权/PII)       |
|  MemoryStore | smartResolve/smartOrder                     |
+------------------------------------------------------------+
                  |
+-- 基础设施层 -----------------------------------------------+
|  order-service :8001 | product-service :8002                |
|  logistics-service :8003 | MySQL 8.0 | Redis 7             |
|  Spring Actuator | Springdoc Swagger | SlaRecorder         |
+------------------------------------------------------------+
```
...
```
+-- Kubernetes Cluster -------------------------------------+
|  Namespace: campus-prod        | Jenkins CI/CD            |
|  campus-server x2 (HPA 2-10)   | Git -> Build -> Registry |
|  order/product/logistics x2    | -> kubectl apply         |
|  MySQL PVC 5Gi + Redis         |                          |
+------------------------------------------------------------+
```
...
```
Start -> Task_QueryLogistics -> Gw_HasMapData
           +- Yes -> Task_BuildRouteMap (GeoJSON)
           +- No  -> End
      -> End
```
